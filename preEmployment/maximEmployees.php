...
function isidNumberExists($con, $idNumber)
{
    ...
    $query = "SELECT COUNT(*) AS count FROM employeespersonalinfo WHERE idNumber = '$idNumber' AND `employer` ='Maxim'";
...
function saveToDatabase($con, $data, $count)
{
    ...
    foreach ($data as $row) {
        if ($count > 0) {
            ...
            $result = mysqli_query($con, "SELECT `Name`, `section` FROM `employeespersonalinfo` WHERE `idNumber` = '$idNumber' AND `employer` ='Maxim'");
                    ...
                    <table id="queTable" class="display text-[9px] 2xl:text-sm" style="width:100%">
                        ...
                        <tbody>
                            ...
                            $sql = "SELECT p.*, e.employer, e.Name, e.section, e.idNumber FROM preemployment p 
                                    JOIN employeespersonalinfo e ON e.idNumber = p.idNumber WHERE e.employer = 'Maxim' ORDER BY `id` ASC";
            ...
            <div class="text-[9px] 2xl:text-lg  rounded-lg bg-white/50 grid grid-cols-4 gap-1 w-full w-full p-4 ">
                <div class="col-span-4 gap-4">
                        ...
                        <select id="name" name="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-[12px] 2xl:text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block  p-2.5 ">
                            ...
                            <?php
                            $sql1 = "SELECT * FROM employeespersonalinfo e WHERE e.employer = 'Maxim' AND e.idNumber NOT IN (SELECT p.idNumber FROM preemployment p);";
