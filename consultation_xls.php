...
header("Content-Disposition: attachment; filename=Consultation Report for the Month of " . $month . ".xls");  //File name extension was wrong
header("Expires: 0");
...
LEFT JOIN `employeespersonalinfo` e ON e.idNumber = c.idNumber WHERE MONTH(c.date) = '$monthNumber'
    AND YEAR(c.date) = '$year' AND c.status = 'done' ORDER BY `id` ASC ");
...
<body>
    <center>
        ...
        <br>
        <h3> <b> Consultation for the Month of <?php echo $month ?></b></h3>
